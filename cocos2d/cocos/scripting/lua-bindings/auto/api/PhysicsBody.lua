...
--------------------------------
--  Whether this physics body is affected by the physics world钬檚 gravitational force. 
...
-- When two physics bodies contact each other, a collision may occur. This body钬檚 collision mask is compared to the other body钬檚 category mask by performing a logical AND operation. If the result is a non-zero value, then this body is affected by the collision. Each body independently chooses whether it wants to be affected by the other body. For example, you might use this to avoid collision calculations that would make negligible changes to a body钬檚 velocity.<br>
-- param bitmask An interger number, the default value is 0xFFFFFFFF (all bits set).
...
-- When two bodies share the same space, each body钬檚 category mask is tested against the other body钬檚 contact mask by performing a logical AND operation. If either comparison results in a non-zero value, an PhysicsContact object is created and passed to the physics world钬檚 delegate. For best performance, only set bits in the contacts mask for interactions you are interested in.<br>
...
-- param   offset A Vec2 object, it is the offset from the body钬檚 center of gravity in body local coordinates.<br>
